...
Trait Crud
{
    ...
    */
    ...
    protected function setData($model, $data){
        try{
            ...
            foreach ($data as $key => $value) {
                ...
                /* Ambil dari session */
                if($model['table'] != 'Dokumen_M'){
                    $model->{'KdProfile'} = $model['table'] == 'Profile_M' ? $last_no + 1 : 1;
                }
                $model->{'NoRec'} = substr(Uuid::generate(), 0, 32);
                if($model->{$key} == 'KdDepartemen'){
                    $model->{'KdDepartemen'} = 1;
                }
                $model->{'version'} = 1;
                $model->{'StatusEnabled'} = 1;
                
                if(@$model['primaryKey'][1]){
                    $model->{$model['primaryKey'][1]} = $last_no + 1;
                }
                $model->{$key} = $value;
            }
            return $model;
    ...
    protected function transformValidation($class){
        $data =  new $class;
    ...
    protected function update_sequenceTbl($table,$KdProfile){
        ...
        if(!empty($KdProfile)){
            ...
            DB::table('SequenceTable_M')->where('namaTabel', $table)->where('KdProfile',$KdProfile)->update([
                                'idTerahir' => $last_no + 1]);    
        } else {
            DB::table('SequenceTable_Ms')->where('namaTabel', $table)->update([
                                'idTerahir' => $last_no + 1]);
        }       
        return $last_no;
    ...
    protected function saveCreate($request){
        ...
        $data = $this->transformValidation($class);
            ...
            if ($validator->fails()) {
                ...
                try{
                    if(substr($newdata['table'],-1) != 'T'){
                        ...
                        $update = DB::table($newdata['table'])->where($newdata['primaryKey'][$index], $request->input($newdata['primaryKey'][$index]))->update($dt);
                    }else{
