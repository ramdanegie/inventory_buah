...
@Component({
  ...
  styleUrls: ['./penerimaan-barang-fix.component.scss'],
	providers: [ConfirmationService]
})
export class PenerimaanBarangFixComponent implements OnInit {
	formGroup: FormGroup;
	listToko: SelectItem[];
	...
	ngOnInit() {
      ...
      'ttlTransaksi': new FormControl(null),
		});
  ...
  onChangeTtl(value: number) {
		let kdProduk = this.formGroup.get('namaProduk').value.kdProduk
    let kdSatuan = this.formGroup.get('satuan').value.id
    let noPenerimaan = this.formGroup.get('')
		if (kdSatuan) {
			this.httpService.get('master/mapproduktosatuan/get?produkfk=' + kdProduk
				+ '&satuantujuanfk=' + kdSatuan).subscribe(res => {
					if (res.data.length > 0) {
    ...
    this.httpService.get('transaksi/penerimaan/get-ttl-penerimaan?produkfk=' + e.kdProduk
    +'&penerimaanfk='+ this.formGroup.get('noRec').value).subscribe(data=>{
      this.formGroup.get('ttlTransaksi').setValue(data.data[0].ttltransaksi)
    })
	}
	simpan() {
